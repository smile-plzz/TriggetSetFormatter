# Build a single-file index.html version that runs with React + Tailwind via CDNs.
...
<head>
  ...
  <title>SPW Formatter</title>
  <meta name="description" content="Bulk trigger-set parser for migration"/>
  <!-- Tailwind via CDN -->
...
<body class="min-h-screen bg-slate-50 text-slate-900">
  ...
  <!-- Babel standalone so we can keep JSX in one file -->
  ...
  <script type="text/babel">
    ...
    function App() {
      ...
      const [sets, setSets] = useState([]); // array of trigger sets
      ...
      const allCollections = useMemo(() => {
        const set = new Map();
        for (const r of flatVariants) (r.product_collections || []).forEach((c) => set.set(c.id, c.title));
        return Array.from(set, ([id, title]) => ({ id, title })).sort((a, b) => a.title.localeCompare(b.title));
      }, [flatVariants]);

      const allTags = useMemo(() => {
        const set = new Set();
        for (const r of flatVariants) (r.product_tags || []).forEach((t) => set.add(t));
        ...
        if (q.trim()) {
          ...
          rows = rows.filter((r) =>
            [
              ...
              ...(r.product_tags || []),
              ...(r.product_collections || []).map((c) => c.title),
            ]
              .filter(Boolean)
              .some((s) => String(s).toLowerCase().includes(needle))
          );
        }
        if (collection) rows = rows.filter((r) => (r.product_collections || []).some((c) => c.id === collection));
      ...
      const exportCSV = () => {
        ...
        for (const r of filtered) {
          const row = [
            ...
            r.inventory_policy ?? "",
            (r.product_collections || []).map((c) => c.title).join("; "),
            (r.product_tags || []).join("; "),
          ].map(toCSV);
          lines.push(row.join(","));
        }
...
"""
path = "/mnt/data/spw-formatter-index.html"
